import { IAuraClinicalDocOutputSectionNew, IEvidenceNew } from '@/types/HealthScribe';
 ...
 */
export function processSections(sections: IAuraClinicalDocOutputSectionNew[]) {
    return sections.map((s) => {
        if (s.SectionName === 'PLAN') {
            let currentPlanHeader: string = 'null';
            const updatedSummary: { [header: string]: IEvidenceNew[] } = {};
